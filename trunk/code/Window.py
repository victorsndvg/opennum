...
#Se utiliza como valor de partida para indexar el resto de menus
        ...
        self.MaterialsDB_exists = False
        # flag para la comprobacion de existencia de config
        self.ConfigFile_exists = False
        ...
        self.reload_menu = False

        if self.configdir is None:
            self.errormsg(u'Error: Could not create config dir. Some functionality may be affected')
...
#carga automatica del ultimo directorio de trabajo
    ...
    def calculate_appsdir(self,apps_dir=None):
        ...
        if os.path.isdir(dirappshome):
            ...
            appsdir = os.path.join(self.path_exe, os.pardir, apps_dir)
        return appsdir
    ...
    def interface_pre1(self):
        ...
        self.item_o = wx.MenuItem(self.menu_folder, self.ID_OPEN, self.folder_name+'\tCtrl+F', u'Select a working folder')
...
#                    (selected,kind) = False,wx.ITEM_RADIO
                ...
                if (attribs.get(u'separator') == u'true'):
                    menu_temp.AppendSeparator()
                    continue
#--------------------------------------------------------------------------------------------------------------------------------------------------------
                #Permite a los submenus mostrar como text el tag title
                if (attribs.get(u'title') == None):
                    item_name = attribs.get(u'name')
                else:
                    item_name = attribs.get(u'title')
#--------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                for subsubmenu in submenu.get_children():
                    ...
                    if (subattribs.get(u'selected') == u'true'):
                        (selected,kind) = True,wx.ITEM_CHECK
                    if (subattribs.get(u'separator') == u'true'):
                        item_sample_temp.AppendSeparator()
                        continue
                #Permite a los submenus mostrar como text el tag title
                    if (subattribs.get(u'title') == None):
                        subitem_name = subattribs.get(u'name')
                    else:
                        subitem_name = subattribs.get(u'title')
                    # Si es menu help trabajamos con otras ids
                    if help:
                    ...
                    item_sample_temp.AppendItem(subitem_temp)
    ...
    def interface_in(self):
        ...
        for menu in menus:
            ...
            if (menuattribs.get(u'separator') == u'true'):
#                pos = self.menubar.GetMenuCount()
#                menu_sep = wx.Menu()
#                self.menubar.Append(menu_sep, u'|')
#                self.menubar.EnableTop(pos, False)
...
#-------------------------------------------------------------------------------------
                #Permite a los submenus mostrar como text el tag title
                if (attribs.get(u'title') is None):
                    item_name = attribs.get(u'name')
                else:
                    item_name = attribs.get(u'title')
#-------------------------------------------------------------------------------------
    ...
    def copy_config(self, force=False):
        try:
            if self.path_local_config is not None:
                ...
                #'File not found. Some functionality may be affected.')
                            #print u'Error copying configuration file to local folder. '+\
                #'File not found. Some functionality may be affected.' #code prior version 0.0.1
                            logging.warning(u'Error copying configuration file to local folder. File not found. Some functionality may be affected.')
                            return
            ...
            self.errormsg(u'Error copying configuration file to local folder. '+\
                ...
                if force or not os.path.exists(self.path_local_materials):
                    ...
                    if os.path.exists(from_file1):
                        ...
                        if os.path.exists(from_file2):
                            ...
                            #'File not found. Some functionality may be affected.' #code prior version 0.0.1
                            logging.warning(u'Error copying materials database to local folder. File not found. Some functionality may be affected.')
                            return
            ...
            if self.path_local_set(path_new):
                ...
                if os.path.exists(config.FILE_MENULOCAL):
                    if not self.menu_load(True):
                        self.menu_postload()
                    # There could be different remote data in each working folder. We need to reload them.
                    self.load_remotedata()
                    return True
                # in case of failure
                else:
...
#                    if len(self.apps2.get()) == 1: # one application => load it
#                        print self.apps2.get()[0][3]
#                        if len(self.apps2.get()[0][3]) == 1:
#                            if self.menu_copy2(self.apps2.get()[0][2]+u'/'+self.apps2.get()[0][3][0][2]):
#                                if self.menu_load(True):
#                                    return True
#                    else:
#                        self.menu_postload()
...
#Carga automatica del ultimo directorio de trabajo
    def load_lastfolder(self):
        ...
        if os.path.exists(path_file_lf) and self.configdir is not None:
            ...
            path_new = file1.read()
            file1.close()
            #print path_new  #code prior version 0.0.1
                ...
                if submenu is not None:
                    if submenu.get_attribs()[config.AT_HELP] == config.VALUE_TRUE:
                        try:
                            ...
                            dirname = submenu.get_attribs()[u'name']
                        if not self.launch_auto(os.path.join(dirname, u'index.xhtml')):
                            if not self.launch_auto(os.path.join(dirname, u'index.html')):
                                if not self.launch_auto(os.path.join(dirname, u'index.htm')):
                                    self.errormsg(u'File '+dirname+u'/'+u'index.[xhtml | html | htm] not exists.')
##############################################################33
# Deprecated actions
...
#                        result = self.copy_mnu(action[1])
                    ...
                    elif name == u'exec_qsub':
                        self.menu_exec_qsub(action)
                    elif name == u'status_qsub':
                        self.menu_status_qsub(action)
                    elif name == u'get_qsub':
...
#                self.launch('file://'+path2)
    ...
    def write_gr2_from_struct(self,struct):
        ...
        if signals is not None:
            sigs = signals.get(u'signals')
            try:
                filegr2 = open(signals.get(u'filename'),'w')
                filegr2.write(str(len(sigs))+'\n')
                for sig in sigs:
                    xvalues = sig.get(u'xaxis').get(u'values')
                    yvalues = sig.get(u'yaxis').get(u'values')
                    if len(xvalues)<= 0 or len(yvalues) <=0:
                        self.errormsg(u'Error showing plot: No scalar data to plot.')
                        return None
                    filegr2.write(str(len(xvalues)) + u'\n')
                    for v in xvalues:
                        filegr2.write(str(v) + u' ')
                    filegr2.write(u'\n')
                    for v in yvalues:
                        filegr2.write(str(v) + u' ')
                    filegr2.write(u'\n')
                ...
                #print u'Error writing data to temporary file: ' + filegr2 #code prior version 0.0.1
                logging.warning(u'Problem writing data to temporary file:'+u' '+unicode(filegr2))
                return None
        return signals.get(u'filename')
    ...
    def graph2d_from_struct(self,struct):
        ...
        if filename is not None:
            ...
            try:
                ...
                #print u'Error deleting temporary file: ' + filename #code prior version 0.0.1
        ...
        try:
            if self.path_local_remotedata is not None:
                if force or not os.path.exists(self.path_local_remotedata):
                    logging.debug(u'Copying remote data file to:'+u' '+unicode(self.path_local_remotedata)+u'...')
                    ...
                    if os.path.exists(from_file1):
                        ...
                        logging.warning(u'Error copying remote data file to local folder. File not found. Some functionality may be affected.')
                        return

                    shutil.copy2( from_file, self.path_local_remotedata )
        except (IOError, shutil.Error, OSError), err:
            logging.debug(repr(err))
            self.errormsg(u'Error copying remote data file to local folder. Some functionality may be affected')
    ...
    def load_remotedata(self):
        ...
        try:
            ...
            logging.debug(u'Problem loading remote data: '+unicode(repr(why)))
            return
...
#                return
    ...
    def menu_exec_qsub(self, action):
        ...
        if self.taskscurrent>=0 and self.taskscurrent<len(self.taskssave):
            self.errormsg(u'There are pending processes to run. Will not start a thread')
            return
