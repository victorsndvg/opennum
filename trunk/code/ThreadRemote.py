...
class ThreadRemote(threading.Thread):
	...
	if self.window.MaterialsDB_exists:						#añadido		
	    matfile = config.FILE_MATERIALS_DAT						#añadido
	    matfilebase = os.path.basename(matfile)					#añadido
	    newmatfile = matfilebase + '.temp-remote.xml'				#añadido
	    self.materials.save_data(newmatfile, force=True ,transform=names)		#añadido
	...
	if self.window.MaterialsDB_exists:							#añadido
            tocopy.append( (newmatfile,								#añadido
            	    serverdirsep.join([self.tempdir,dirfiles,config.FILE_MATERIALS_DAT]))	#añadido
                     )
